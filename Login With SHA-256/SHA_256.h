//This is a lesson made after hak8or's implementation.
//The original : https://gist.github.com/hak8or/8794351
//Also, this document : https://csrc.nist.gov/csrc/media/publications/fips/180/4/final/documents/fips180-4-draft-aug2014.pdf
...
vector<unsigned long> resize_block(const vector<unsigned long>);
...
// //Debug :
    ...
    void count_block_state(vector<unsigned long>);
    ...
    const bool show_T1_calculations = true;
    const bool show_T2_calculations = true;
    const bool show_hash_segments = true;
    const bool show_Wt = true;

/*
    Applies to the individual bits :

    &  - bitwise AND
    |  - bitwise OR
    ^  - bitwise XOR
    ~  - bitwise NOT
    << - bitwise shift left     //Not for negative numbers
    >> - bitwise shift right    //Not for negative numbers

    Example :
        a = 5 (00000101), b = 9 (00001001)

        a & b = 00000001 (only one in common)
        a | b = 00001101 (if any bit is 1, than 1.)
        a ^ b = 00001100 (either but not both. Basically reverse of AND)
        ~ a   = 11111010 (reverse of a)
        b << 1 = 00010010 (move everything to the left by 1)
        b >> 1 = 00000100 (move right by 1)

        The bit shift operators are equivalent to multiplication and division by 2 :

        a << 1 = 10
        a >> 1 = 3
*/

            //Learning bitwise operations is so much fun, I want to die!

    #define ROTRIGHT(word,bits) (((word) >> (bits)) | ((word) << (32-(bits))))
            //          shift word right by bits    OR   shift left by 32-bits 
    #define SSIG0(x) (ROTRIGHT(x,7) ^ ROTRIGHT(x,18) ^ ((x) >> 3))
            //      shift x by 7   XOR   shift x by 18    XOR  shift x 3 left
    #define SSIG1(x) (ROTRIGHT(x,17) ^ ROTRIGHT(x,19) ^ ((x) >> 10))
            //      shift x by 17   XOR  shift x by 19   XOR   shift x by 10 left
    #define CH(x,y,z) (((x) & (y)) ^ (~(x) & (z)))
            //            AND    XOR  NOT  AND
    #define MAJ(x,y,z) (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)))
            //              AND     XOR   AND     XOR    AND

    // BSIG0 is replaced by EP0 and BSIG1 is replaced with EP0
...
string EncryptFunction(string message) {
    ...
    // cout << "\n\n Message toEncrypt in SHA : " << message; 
    block = convert_to_binary(message);
    block = pad_to_512bits(block);
    block = resize_block(block);
    string hash = compute_hash(block);
...
vector<unsigned long> resize_block(vector<unsigned long> input) {
    ...
    for(int i = 0; i < 64; i = i + 4){
        ...
        //Put the new 32 bit word into the correct output array location :
...
//DEBUG :
//COUT BLOCK STATE :
//Shows the current contents of all blocks in binary for debugging
...
vector<unsigned long> convert_to_binary(const string input) {
    //vector to hold all AHCII values :
    vector<unsigned long> block;


    for (int i = 0; i < input.size(); i++){

        //temp var to hold AHCII value :
        bitset<8> b(input.c_str()[i]);
        //add the 8 bit pattern into the block :
...
// PAD TO 512 BITS :
//Takes the vector of AHCII values in binary and pad it so that there will be a total of 512 bits.

//Padding basically adds 1 to the end of the message and than adds 0's till the length is 448 bits
//than it adds 64 bits witch say how long the original message was, giving a total of 512 bits.

vector<unsigned long> pad_to_512bits(vector<unsigned long> block) {
    //l is length of message where each block is one AHCII character
    int l = block.size() * 8;

    //Padding equation : l + 1 + k = 448 mod 512
    ...
    //just added in 7 zero's, so subtract 7 from k.
    k = k - 7;
    ...
    //It is now at 488 bits out of 512. We need to add l :

    bitset<64> big_64bit_blob(1);
    
    if(show_distance_from_512bit)
        cout << "l is a 64 bit binary blob : \n\t" << big_64bit_blob << endl;

    //Split the blob into 8 bit sections :
    string big_64bit_blob_string = big_64bit_blob.to_string();

    bitset<8> temp_string_holder1(big_64bit_blob_string.substr(0,8));
    block.push_back(temp_string_holder1.to_ulong());

    for(int i = 8; i < 63; i += 8) {
        bitset<8> temp_string_holder2(big_64bit_blob_string.substr(i,8));
...
//COMPUTE HASH :
//Input : the 512 bit padded message as a vector containing 5 bit binary ASCII values.
//Output : A string representing the hash.

string compute_hash(const vector<unsigned long> block) {

    //SHA-256 uses the same sequence of 64 costant 32-bit words, k0 - k 6. Those words represent
    //the first 32 bits of thefractional parts of the cube roots of the first sixty four prime numbers.
    unsigned long k[64] {
    ...
    //Initial hash values, first 32 bits of the frational parts of the square roots of the first 8 prime num
    ...
    unsigned long static H6 = 0x1f83f9ab;
    ...
    for (int t = 0; t < 64; t++) {
        ...
        if((t == 20) & show_T1_calculations) {
            
            cout << "\nh: 0x" << hex << h << "  dec:" << dec << h 
			     << "  sign:" << dec << (int)h << endl;
			cout << "EP1(e): 0x" << hex << BSIG1(e) << "  dec:" 
			     << dec << BSIG1(e) << "  sign:" << dec << (int)BSIG1(e) 
			     << endl;
        ...
        if((t == 20) & show_T2_calculations) {
			...
			cout << "c: 0x" << hex << c << "  dec:" << dec << c 
			     << "  sign:" << dec << (int)c << endl;
			cout << "EP0(a): 0x" << hex << BSIG0(a) << "  dec:" 
			     << dec << BSIG0(a) << "  sign:" << dec << (int)BSIG0(a) 
			     << endl;
